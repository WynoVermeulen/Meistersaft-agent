...
function contactRecencyLabel(customerId) {
  ...
  if (!info) return "Nog nooit contact";
  return `${info.datum} (${info.daysSince} dagen geleden)`;
}
...
function renderRecencyList(containerId, countId, customers) {
  ...
  document.getElementById(countId).textContent = `${rows.length} klanten`;
  const el = document.getElementById(containerId);
  if (rows.length === 0) {
    el.innerHTML = `<div class="empty">Geen klanten gevonden.</div>`;
    return;
  }
  el.innerHTML = rows
    .map(
    ...
    <div class="dash-list-row">
      ...
      <span>${r.info ? escapeHtml(r.info.datum) + ` <span class="dl-sub">(${r.info.daysSince}d)</span>` : `<span class="dl-sub">nog nooit</span>`}</span>
    </div>`
    )
    .join("");
}
...
function renderCustomers() {
  ...
  const html = `
    <table>
      <thead>
        <tr>
          <th>Klant</th><th>Kanaal</th><th>ABC</th><th>Status</th><th>Plaats</th><th>Telefoon</th><th>E-mail</th><th>Laatste contact</th>
        </tr>
      </thead>
      <tbody>
        ${rows
...
document.getElementById("clm-save-btn").addEventListener("click", async () => {
  ...
  if (clmRueckruf.value === "ja" && clmCallbackDatum.value && clmCallbackTijd.value && clmCurrentCall) {
    ...
    const { error: cbError } = await sb.from("phone_call_tasks").insert({
      ...
      auftrag_typ: "Terugbelafspraak",
      gespraechsziel: "Terugbelafspraak n.a.v. vorig gesprek",
      dauer_min: 10,
      status: "geplant",
    });
    if (cbError) {
      clmSaveStatus.textContent = "Opgeslagen, maar terugbelafspraak inplannen mislukt: " + cbError.message;
      clmSaveStatus.className = "vm-hint err";
      await loadCalls();
      return;
    }
