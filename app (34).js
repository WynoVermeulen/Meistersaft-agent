...
const I18N = {
  nl: {
    ...
    kb_titel: "Beschikbaarheid voor demo's (komende 30 dagen)",
    kb_hint: "Vink de dagen aan waarop je NIET beschikbaar bent voor een demo.",
    kb_beschikbaar: "Beschikbaar",
    kb_niet_beschikbaar: "Niet beschikbaar",
    ac_vandaag_titel: "Vandaag te bellen",
    ac_dringend_titel: "Dringend — lang geen contact",
    ac_kan_wachten_titel: "Kan nog even wachten",
    ac_bel_nu: "Openen →",
  },
  de: {
    ...
    kb_titel: "Verfügbarkeit für Demos (nächste 30 Tage)",
    kb_hint: "Markiere die Tage, an denen du NICHT für eine Demo verfügbar bist.",
    kb_beschikbaar: "Verfügbar",
    kb_niet_beschikbaar: "Nicht verfügbar",
    ac_vandaag_titel: "Heute anzurufen",
    ac_dringend_titel: "Dringend — lange kein Kontakt",
    ac_kan_wachten_titel: "Kann noch warten",
    ac_bel_nu: "Öffnen →",
  },
};
...
async function loadActionsTodayBlock() {
  const block = document.getElementById("actions-today-block");
  const listEl = document.getElementById("actions-today-list");
  if (!block) return;

  const today = new Date().toISOString().slice(0, 10);
  const { data, error } = await sb
    .from("phone_call_tasks")
    .select("telefon_id, kunde, zeitfenster, gespraechsziel, status")
    .eq("geplantes_telefon_datum", today)
    .not("status", "in", "(voltooid,gebeld)");
  ...
  listEl.innerHTML = data
    .map(
      (c) => `
    <div class="dash-list-row" data-telefon-id="${escapeHtml(c.telefon_id)}" style="cursor:pointer;">
      <span class="dl-name">${escapeHtml(c.kunde || "—")} <span class="dl-sub">${escapeHtml(c.zeitfenster || "")} · ${escapeHtml(c.gespraechsziel || "")}</span></span>
      <span>${t("ac_bel_nu")}</span>
    </div>`
    )
    .join("");

  listEl.querySelectorAll("[data-telefon-id]").forEach((row) => {
    row.addEventListener("click", () => {
      const telefonId = row.dataset.telefonId;
      const call = data.find((c) => c.telefon_id === telefonId);
...
function wireActionRowButtons(container) {
...
function renderActionsList() {
  ...
  // Dringend: nooit contact gehad, of meer dan 60 dagen geleden.
  // Kan nog wachten: tussen de 31 en 60 dagen geleden.
  const dringend = rows.filter((r) => r.lastContact === null || r.daysSince > 60);
  const kanWachten = rows.filter((r) => r.lastContact !== null && r.daysSince <= 60);

  let html = "";
  if (dringend.length > 0) {
    html += `<h3 class="dash-section-title">${t("ac_dringend_titel")} (${dringend.length})</h3>`;
    html += dringend.map(actionRowHtml).join("");
  }
  if (kanWachten.length > 0) {
    html += `<h3 class="dash-section-title" style="margin-top:18px;">${t("ac_kan_wachten_titel")} (${kanWachten.length})</h3>`;
    html += kanWachten.map(actionRowHtml).join("");
  }
  listEl.innerHTML = html;
  wireActionRowButtons(listEl);
}
...
// Strenger dan findCustomerIdByName: vereist ook overeenkomende postcode,
// zodat filiaalketens (bv. meerdere EDEKA Paschmann-vestigingen) niet door elkaar lopen.
function findCustomerIdByNameAndPostcode(kunde, postcode) {
  if (!kunde || !postcode) return null;
  const cleanPc = postcode.replace(/\s/g, "");
  const kandidaten = allCustomers.filter((c) => (c.postcode || "").replace(/\s/g, "") === cleanPc);
  ...
  kandidaten.forEach((c) => {
    const overlap = naamTokens(c.kunde).filter((w) => taskTokens.has(w)).length;
    if (overlap > bestScore) {
      bestScore = overlap;
      best = c;
    }
...
async function loadBeschikbaarheid() {
  const el = document.getElementById("kb-kalender");
  if (!el) return;

  const today = new Date();
  const eind = new Date();
  eind.setDate(eind.getDate() + 29);
  const startStr = today.toISOString().slice(0, 10);
  const eindStr = eind.toISOString().slice(0, 10);

  const { data, error } = await sb
    .from("kasia_beschikbaarheid")
  ...
  const onbeschikbaarSet = new Set((data || []).filter((r) => r.beschikbaar === false).map((r) => r.datum));
  ...
  el.innerHTML = dagen
    .map((datum) => {
      const unavailable = onbeschikbaarSet.has(datum);
      const d = new Date(datum + "T00:00:00");
      const label = d.toLocaleDateString(currentLang === "de" ? "de-DE" : "nl-NL", { weekday: "short", day: "numeric", month: "short" });
      return `
      <div class="kb-day ${unavailable ? "unavailable" : ""}" data-datum="${datum}">
        <div class="kb-date">${escapeHtml(label)}</div>
        <div class="kb-label">${unavailable ? t("kb_niet_beschikbaar") : t("kb_beschikbaar")}</div>
      </div>`;
    })
    .join("");

  el.querySelectorAll(".kb-day").forEach((dayEl) => {
    dayEl.addEventListener("click", async () => {
      const datum = dayEl.dataset.datum;
      const nowUnavailable = !dayEl.classList.contains("unavailable");
      dayEl.classList.toggle("unavailable");
      dayEl.querySelector(".kb-label").textContent = nowUnavailable ? t("kb_niet_beschikbaar") : t("kb_beschikbaar");

      await sb.from("kasia_beschikbaarheid").upsert(
        {
          datum,
          beschikbaar: !nowUnavailable,
          bijgewerkt_door: currentUser?.email || null,
  ...
  try {
    if (item.herkend_type === "grovisto") {
      ...
      for (const b of bezoeken) {
        ...
        const matchedId = postcode ? findCustomerIdByNameAndPostcode(b.klant_naam, postcode) : null;
        ...
        if (matchedId) {
          await sb.from("customer_contacts").insert({
            ...
            soort: "bezoek",
            besproken: [b.actie, b.verslag].filter(Boolean).join(" — "),
            door: "Grovisto",
          });
        }
      }
