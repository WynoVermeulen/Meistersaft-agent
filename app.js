...
async function loadSalesSection(customer) {
  ...
  const { data, error } = await sb
    .from("sales_2025")
    .select("*")
    .eq("relid", customer.linked_prodin_relid)
    .maybeSingle();

  if (error) {
    el.innerHTML = `<div class="empty">Kon omzetdata niet laden: ${escapeHtml(error.message)}</div>`;
    return;
  }
  if (!data) {
    el.innerHTML = `<div class="empty">Gekoppeld aan relid ${customer.linked_prodin_relid}, maar geen omzetregel gevonden voor 2025.</div>
      <button class="sales-unlink" id="cm-unlink-btn">Koppeling verwijderen</button>`;
  } else {
    el.innerHTML = `
      <div class="sales-figures">
        <div class="fig"><div class="fig-val">€${data.netto_omzet.toFixed(0)}</div><div class="fig-label">Netto omzet</div></div>
        <div class="fig"><div class="fig-val">${data.aantal}</div><div class="fig-label">Aantal verkocht</div></div>
        <div class="fig"><div class="fig-val">${data.bruto_marge_pct.toFixed(1)}%</div><div class="fig-label">Bruto marge</div></div>
      </div>
      <button class="sales-unlink" id="cm-unlink-btn">Koppeling verwijderen</button>`;
  }
